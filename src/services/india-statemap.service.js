...
class DataManger {
  ...
  moveTo = (index) => {
    if (index >= 0 && index < this.totalElements) this.state = this.dataStore[index]
  }
  isEnded = () => {
...
const StateDataManager = () => {
  ...
  const mapColorThemes = {
    light: {
      ...
      active: [
        { percent: 0.0, color: { r: 246, g: 231, b: 206 } },
        { percent: 1.0, color: { r: 255, g: 156, b: 0 } },
      ],
      deaths: [
        { percent: 0.0, color: { r: 216, g: 216, b: 216 } },
        { percent: 1.0, color: { r: 71, g: 71, b: 71 } },
      ],
      recovered: [
        { percent: 0.0, color: { r: 196, g: 239, b: 175 } },
    ...
    ;[
      ...
      dataStore.forEach((dayObject) => {
        ...
        Object.keys(statewise).forEach((stateKey) => {
          const percent =
            statewise[stateKey].value[attributeKey] / maxObject[attributeKey]
          if (percent === 0) {
          ...
          var color = {
            r: Math.floor(
              lower.color.r * percentLower + upper.color.r * percentUpper
            ),
            g: Math.floor(
              lower.color.g * percentLower + upper.color.g * percentUpper
            ),
            b: Math.floor(
              lower.color.b * percentLower + upper.color.b * percentUpper
            ),
          }
          stateWiseColorsMap[stateKey] =
            'rgb(' + [color.r, color.g, color.b, opacity].join(',') + ')'
        })
  ...
  return {
    ...
    moveTo: (index) => {
      if (index >= 0 && index < totalElements) state = dataStore[index]
    },
    isEnded: () => {
