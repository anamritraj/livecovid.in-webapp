...
const showContentUpdatedHTML = (
  <span>
    Website was updated in background, please reload to load the lastest version.{' '}
...
const App = () => {
  ...
  if (!componentMounted || isLoading) return <Loading />
        ...
        <Suspense fallback={<Loading></Loading>}>
          <>
            <Toolbar
              handleDarkModeClick={() => {
                toggleTheme()
                sendEventToGA('User', 'Dark/Mode', theme)
              }}
              theme={theme}
            />
          ...
          <Alert
            show={showContentUpdatedAlert}
            content={showContentUpdatedHTML}
          ></Alert>
        </Suspense>
      </Router>
    </>
