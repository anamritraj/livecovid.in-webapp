...
const IndiaMapStats = ({ stats, handleAttributeClick, activeAttribute, mode }) => {
    ...
    <>
      <div className="india-state-maps">
        ...
        >
          <h5>
            {Number(
              stats[isNormal ? 'confirmed' : 'confirmedPerMillion']
            ).toLocaleString('en')}
          </h5>
          {isNormal && (
            <h6>+{Number(stats.delta.confirmed).toLocaleString('en')}</h6>
          )}
          <h4>{t('Confirmed')}</h4>
        ...
        >
          <h5>
            {Number(stats[isNormal ? 'active' : 'activePerMillion']).toLocaleString(
              'en'
            )}
          </h5>
          <h4>{t('Active')}</h4>
        ...
        >
          <h5>
            {Number(
              stats[isNormal ? 'recovered' : 'recoveredPerMillion']
            ).toLocaleString('en')}
          </h5>
          {isNormal && (
            <h6>+{Number(stats.delta.recovered).toLocaleString('en')}</h6>
          )}
          <h4>{t('Recovered')}</h4>
          {!isNormal && <h6>{t('per million')}</h6>}
        </motion.div>
        ...
        >
          <h5>
            {Number(stats[isNormal ? 'deaths' : 'deathsPerMillion']).toLocaleString(
              'en'
            )}
          </h5>
      ...
      <div className="instruction">
        {t('Click on the tiles to see cases in that category')}
      </div>
    </>
  )
}
