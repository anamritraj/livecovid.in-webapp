...
const promptMsg = (
  <p>
    You need to allow permissions to get Notifications. When prompted click 'Allow'
    to get notifications
  </p>
)
const blockedMsg = (
  <p>
    Seems like you have denied the message permissions, you need to click "Allow" to
    give permissions for notifications.
    <a
      href="https://support.google.com/chrome/answer/3220216?co=GENIE.Platform%3DAndroid&hl=en&oco=1"
      target="_blank"
...
const errorMsg = (
  <p>
    There was an error, please try again. It is possible that your browser doesn't
    support notifications.
  </p>
)
const unsupportedMsg = (
  <p>
    Seems like your browser doesn't support notifications. Make sure you are not in a
...
const StatewiseTable = ({ statewise, statewiseTestingData, isMobile }) => {
  ...
  const sortStates = useCallback(
    (key, order, stateWiseKeys) =>
      stateWiseKeys.sort(
        (a, b) => order * (parseInt(statewise[b][key]) - parseInt(statewise[a][key]))
      ),
    [statewise]
  )
  ...
  const getNotificationPermissions = () => {
    askUserPermissions()
      ...
      .catch((err) => {
        switch (err.msg) {
          case 'blocked':
            setModalContent(blockedMsg)
            break
          case 'browser_unsupported':
            setModalContent(unsupportedMsg)
            break
          default:
      ...
      keys.forEach((key) => {
        switch (key) {
          ...
          default:
            ...
            if (!statesNotificationStatus[state_code])
              statesNotificationStatus[state_code] = {}
  ...
  const handleBellClick = (stateCode, districtName, isBellActive) =>
    subscribeUser(`${stateCode}_${districtName}`, isBellActive)
      .then((msg) => {
          ...
          <thead>
            <tr>
              ...
              <th
                ...
                onClick={(e) => {
                  handleSortClick('confirmed')
                  sendEventToGA(category, action, 'confirmed')
                }}
              >
                <div className="heading">
                  <div className="confirmed">
                    {isMobile ? t('Cnfrm') : t('Confirmed')}
                  </div>
              ...
              <th
                ...
                onClick={(e) => {
                  ...
                  sendEventToGA(category, action, 'active')
                }}
              >
                <div className="heading">
              ...
              <th
                className="clickable-header"
                onClick={(e) => {
                  handleSortClick('recovered')
                  sendEventToGA(category, action, 'recovered')
                }}
              >
                <div className="heading">
              ...
              <th
                className="clickable-header"
                onClick={(e) => {
                  handleSortClick('deaths')
                  sendEventToGA(category, action, 'deaths')
                }}
              >
                <div className="heading">
                  ...
                  <div className="sortIcon">
                    {stateSortObject.activeSorting === 'deaths'
                      ? SortIcon(stateSortObject.currentOrder)
                      : null}
                  </div>
                </div>
          ...
          <Modal>
            <div className="modal text-center">
              ...
              <div className="modal-button-wrapper">
                {showGetNotificationButtons && (
                  <button className="btn" onClick={getNotificationPermissions}>
                    Okay
                  </button>
                )}
